<template>
  <div :style="{paddingLeft: 'auto', paddingRight:'auto'}">
    <FormulateForm v-model="formValues" type="group">
      <fieldset>
        <legend :style="{ color: 'white', marginBottom: '10px' }">Adresse löschen</legend>

        <FormulateInput
          type="text"
          name="id"
          placeholder="123"
          label="ID der zu löschenden Adresse:"
          validation="required"
        ...
        <FormulateInput
          ...
          disabled
        />
        <FormulateInput v-else type="submit" label="Löschen" @click="deleteAddress()" />
      </fieldset>
    </FormulateForm>
  </div>
</template>
  ...
  methods: {
    deleteAddress: async function () {
      ...
      axios
        ...
        .then((res) => {
          alert("Adresse wurde erfolgreich gelöscht");
          console.log(res);
        })
